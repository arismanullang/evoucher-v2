...
type ReportReimburse struct {
	...
	PartnerID        string     `db:"partner_idx" json:"partner_id,omitempty"`
	PartnerName      string     `db:"partner_name" json:"partner_name,omitempty"`
...
func GetReportDailyVoucherTransactionWithPartner(dateFrom, dateTo string, qp *util.QueryParam) ([]ReportReimburse, bool, error) {
	q := `
			SELECT date(v.created_at) as date,
					case
			...
			FROM vouchers v
					 LEFT JOIN transaction_details td on td.voucher_id = v.id AND td.status = 'created'
					 LEFT JOIN transactions t on td.transaction_id = t.id AND t.status = 'created'
					 LEFT JOIN partners p on t.partner_id = p.id AND p.status = 'created'
					 LEFT JOIN cashout_details cd on v.id = cd.voucher_id AND cd.status = 'created'
					 LEFT JOIN cashouts c on cd.cashout_id = c.id AND c.status = 'created'
			WHERE v.created_at BETWEEN '` + dateFrom + ` 00:00:00+07'::timestamp AND '` + dateTo + ` 23:59:59+07'::timestamp 
			  AND v.status = 'created'
			GROUP BY date, partner_idx, partner_name `
...
func GetReportDailyPartnerTransaction(dateFrom, dateTo string, qp *util.QueryParam) ([]ReportOutletTransaction, bool, error) {
	q := `
...
func GetReportMonthlyOutletTransaction(dateFrom, dateTo string, qp *util.QueryParam) ([]ReportOutletTransaction, bool, error) {
	q := `
			SELECT date as transaction_month, id as partner_id, name as partner_name, sum(trans) as transaction_qty, sum(vouc) as voucher_qty, sum(amount) as total_transaction
			FROM (
...
func GetReportYearlyOutletTransaction(dateFrom, dateTo string, qp *util.QueryParam) ([]ReportOutletTransaction, bool, error) {
	q := `
			SELECT date as transaction_year, id as partner_id, name as partner_name, sum(trans) as transaction_qty, sum(vouc) as voucher_qty, sum(amount) as total_transaction
			FROM (
