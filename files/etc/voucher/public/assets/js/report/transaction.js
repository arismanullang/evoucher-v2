        ...
        success: function (data) {
          ...
          for ( i = 0; i < arrData.length; i++){
            ...
            if( tempIndex == -1){
              ...
              dataStart.push(arrData[i].start_date);
              dataEnd.push(arrData[i].end_date);
          ...
          for ( i = 0; i < dataId.length; i++){
            ...
            console.log(dataVoucher[i] > 0);

            if (dataVoucher[i] > 0){
              button = "<button type='button' onclick='detail(\""+dataId[i]+"\")' class='btn btn-flat btn-sm btn-info'><em class='ion-search'></em></button>"+
              "<button type='button' disabled='' onclick='edit(\""+dataId[i]+"\")' class='btn btn-flat btn-sm btn-info'><em class='ion-edit'></em></button>"+
              "<button type='button' disabled='' value=\""+dataId[i]+"\" class='btn btn-flat btn-sm btn-danger swal-demo4'><em class='ion-trash-a'></em></button>";
            }

            dataSet[i] = [
              dataName[i]
              , dataPrice[i]
              , addDecimalPoints(dataValue[i])
